...
export default function RecruiterSettingsPage({
  ...
  return (
    <div className="space-y-6 font-sans text-[#14181C] dark:text-slate-50">
      ...
      <form
        className="rounded-2xl border border-[#14181C]/10 dark:border-white/10 bg-white dark:bg-slate-900 p-6 shadow-sm"
        onSubmit={onSaveProfile}
      >
        <p className="font-data text-xs tracking-[0.2em] text-[#0E7C66]">NOTIFICATION PREFERENCES</p>
        <label className="mt-4 flex cursor-pointer items-center justify-between gap-6 rounded-xl border border-[#14181C]/10 dark:border-white/10 bg-[#F5F6F3] dark:bg-slate-800 px-5 py-4">
          <div>
          ...
          <span className="relative inline-flex shrink-0 items-center">
            ...
            />
            <span className="h-7 w-12 rounded-full bg-[#D9DCD5] transition peer-checked:bg-[#0E7C66] dark:bg-slate-700" />
        ...
        <div className="mt-5 flex items-center gap-4">
          <button
            ...
            className="inline-flex items-center rounded-full bg-[#0E7C66] px-5 py-3 text-sm font-medium text-white transition hover:bg-[#0B6553] disabled:opacity-60"
          >
